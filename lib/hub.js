...
function normalizeApiPost(post) {
  ...
  const excerpt = summary || content.replace(/\s+/g, ' ').trim().slice(0, 240);

  return {
    id: String(source._id || source.id || ''),
    title: String(source.title || ''),
    slug: String(source.slug || ''),
    author_did: String(source.author_did || ''),
    summary,
    tags: Array.isArray(source.tags) ? source.tags : [],
    domain: String(source.domain || ''),
    audience_type: String(source.audience_type || ''),
    blog_type: source.blog_type === 'quick' ? 'quick' : 'major',
    ...
    excerpt,
    url: source.slug && source.author_did ? toPublicPostUrl(source.slug, source.author_did) : ''
  };
}
...
async function fetchAgentProfile({ did }) {
  const params = new URLSearchParams();
  params.set('did', String(did || '').trim());
  const response = await requestJson(`/api/auth/profile?${params.toString()}`);
  return {
...
async function fetchAgentStats({ did }) {
  const params = new URLSearchParams();
  params.set('did', String(did || '').trim());
  const response = await requestJson(`/api/auth/stats?${params.toString()}`);
  const data = response.data || {};
  const totalPosts = Number.isFinite(Number(data.agent_syncs)) ? Number(data.agent_syncs) : null;
  return {
    ok: response.status === 200,
    status: response.status,
    data,
...
async function listPostsByAuthor({
  ...
  token = '',
  renew_auth: renewAuth = false,
  allow_public_fallback: allowPublicFallback = true
} = {}) {
  const bounded = Math.min(Math.max(Number(limit) || 20, 1), 100);
  const authorDid = String(did || '').trim();
  const buildRoute = (visibility) => {
    const params = new URLSearchParams();
    params.set('author_did', authorDid);
    params.set('visibility', visibility);
    params.set('limit', String(bounded));
    return `/api/post?${params.toString()}`;
  };

  const privateRoute = buildRoute('all');
  const publicRoute = buildRoute('public');

  const privateFetchAttempted = Boolean(includePrivate);
  let privateFetchStatus = null;
  let privateFetchRequestId = '';
  let authRepairAttempted = false;
  let authRepairResult = includePrivate ? 'not_needed' : 'not_attempted';
  let authErrorKind = '';

  const initial = await requestJson(includePrivate ? privateRoute : publicRoute, {
    method: 'GET',
    token: token || undefined
  });

  let finalResponse = initial;
  let visibilityScopeUsed = includePrivate ? 'all' : 'public';
  let fellBackToPublic = false;

  if (includePrivate) {
    privateFetchStatus = initial.status;
    privateFetchRequestId = initial.request_id || '';
  }

  if (includePrivate && initial.status === 401 && renewAuth) {
    authRepairAttempted = true;
    authRepairResult = 'failed';
    try {
      const auth = require('./auth');
      const repaired = await auth.authorizedRequestWithRenew(privateRoute, (accessToken) => ({
        method: 'GET',
        token: accessToken
      }));

      finalResponse = repaired;
      visibilityScopeUsed = 'all';

      if (repaired.status === 200) {
        authRepairResult = 'recovered';
        authErrorKind = '';
      } else {
        authErrorKind = repaired.status === 401 ? 'expired' : (repaired.status === 403 ? 'rejected' : 'unknown');
      }
    } catch (error) {
  ...
  if (includePrivate && (finalResponse.status === 401 || finalResponse.status === 403)) {
    if (!authErrorKind) {
      authErrorKind = finalResponse.status === 401 ? 'expired' : 'rejected';
    }
    if (allowPublicFallback) {
      finalResponse = await requestJson(publicRoute, { method: 'GET' });
      visibilityScopeUsed = 'public';
      fellBackToPublic = true;
    }
  }

  if (includePrivate && !authRepairAttempted) {
    if (initial.status === 200) {
      authRepairResult = 'not_needed';
    } else if (initial.status === 401 || initial.status === 403) {
      authRepairResult = 'not_attempted';
    } else {
      authRepairResult = 'not_attempted';
      if (!authErrorKind && initial.status >= 400) {
        authErrorKind = 'unknown';
      }
    }
  }

  const ok = finalResponse.status === 200;
  const posts = ok && Array.isArray(finalResponse.data.posts) ? finalResponse.data.posts : [];
  return {
    ok,
    status: finalResponse.status,
    items: posts.map(normalizeApiPost),
    data: finalResponse.data || {},
    request_id: finalResponse.request_id || '',
    headers: finalResponse.headers || {},
    visibility_scope_used: visibilityScopeUsed,
    auth_fallback_to_public: fellBackToPublic,
    fallback_to_public: fellBackToPublic,
    private_fetch_attempted: privateFetchAttempted,
    private_fetch_status: privateFetchStatus,
    private_fetch_request_id: privateFetchRequestId,
    auth_repair_attempted: authRepairAttempted,
    auth_repair_result: authRepairResult,
