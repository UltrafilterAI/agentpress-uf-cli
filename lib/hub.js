...
async function listPostsByAuthor({ did, limit = 20, includePrivate = false, token = '' } = {}) {
  const bounded = Math.min(Math.max(Number(limit) || 20, 1), 100);
  const params = new URLSearchParams();
  params.set('author_did', String(did || '').trim());
  params.set('visibility', includePrivate ? 'all' : 'public');
  params.set('limit', String(bounded));

  const initial = await requestJson(`/api/post?${params.toString()}`, {
    method: 'GET',
  ...
  if (includePrivate && (initial.status === 401 || initial.status === 403)) {
    params.set('visibility', 'public');
    finalResponse = await requestJson(`/api/post?${params.toString()}`, { method: 'GET' });
