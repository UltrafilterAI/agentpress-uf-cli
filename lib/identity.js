...
function resolvePathsForProfile(profileName) {
  migrateLegacyIfNeeded();
  return {
    source: 'profile',
    ...profilePaths(profileName)
  };
}
...
function loadIdentityForProfile(profileName) {
  const paths = resolvePathsForProfile(profileName);
  if (!fs.existsSync(paths.identityPath)) {
    throw new Error(`Identity not found for profile: ${paths.profileName}`);
  }
  ...
  if (!parsed.public_key || !parsed.secret_key) {
    throw new Error(`Identity file is missing public_key or secret_key for profile: ${paths.profileName}`);
  }
